...
if (!fs.existsSync(envPath)) {
  ...
  try {
    ...
    console.log(`   ❌ 读取 .env 文件失败: ${errorMessage}`);
    allValid = false;
  }
}
...
if (!fs.existsSync(configPath)) {
  ...
  try {
    ...
    if (config.ace?.layers?.l1?.constitutionPath)
      removedConfigs.push("ace.layers.l1.constitutionPath");
    ...
    appConfigs.forEach((configCheck) => {
      const value = configCheck.path.split(".").reduce<unknown>((obj, key) => {
        if (obj && typeof obj === "object" && key in obj) {
          return (obj as Record<string, unknown>)[key];
        }
        return undefined;
      }, config);
      console.log(`      - ${configCheck.desc}: ${value !== undefined ? String(value) : "未配置"}`);
    });
  } catch (error: unknown) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    console.log(`   ❌ JSON 解析错误: ${errorMessage}`);
    allValid = false;
  }
}
