...
async function testConcurrency() {
  ...
  // 并发创建100个会话
  for (let i = 0; i < 100; i++) {
    promises.push(aceCore.createSession(`session_${i}`, { userId: `user_${i}` }));
  }
  ...
  const queryPromises = [];
  for (let i = 0; i < 100; i++) {
    queryPromises.push(Promise.resolve(aceCore.getSession(sessionIds[i])));
  }
  ...
  const readPromises = [];
  for (let i = 0; i < 50; i++) {
    readPromises.push(aceCore.getScratchpad(sessionIds[i], `layer_${i}`));
  }
...
async function testCacheUtilities() {
  ...
  const ttlCache = new TTLCache(50); // 50ms TTL
