...
export class LanceDBConnection implements ILanceDBConnection {
  ...
  async connect(config: LanceDBConfig): Promise<void> {
    try {
      ...
      // Connect to LanceDB
      this.db = await lancedb.connect(config.databasePath);
      this.config = config;
      this.connected = true;
  ...
  private async createTable(): Promise<void> {
    ...
    logger.info(`[LanceDB] Created new table: ${this.config.tableName} with ${this.config.vectorDimensions} dimensions`);
  ...
  private async createVectorIndex(): Promise<void> {
    ...
    try {
      ...
      logger.info('[LanceDB] Created vector index for skills table');
    } catch (error) {
