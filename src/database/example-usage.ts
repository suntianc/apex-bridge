...
async function example1_RunMigrations(): Promise<void> {
  ...
  const dbPath = ".data/llm_providers.db";
...
function example2_QueryDatabase(): void {
  ...
  try {
    ...
    const types = db
      .prepare("SELECT * FROM type_vocabulary ORDER BY confidence DESC LIMIT 5")
      .all();
    types.forEach((type: any) => {
      console.log(
        `  - ${type.tag_name}: confidence=${type.confidence}, count=${type.playbook_count}`
      );
    });
    ...
    assignments.forEach((assignment: any) => {
      console.log(
        `  - Playbook ${assignment.playbook_id}: ${assignment.tag_name} (${assignment.confidence})`
      );
    });
    ...
    console.log("\nPrompt Templates:");
    const templates = db
      .prepare("SELECT * FROM prompt_templates ORDER BY usage_count DESC LIMIT 5")
      .all();
    templates.forEach((template: any) => {
      console.log(
        `  - ${template.name}: type=${template.template_type}, usage=${template.usage_count}`
      );
    });
  } finally {
    db.close();
...
function example3_InsertData(): void {
  ...
  try {
    ...
    `
    ).run("pb_001", "rapid_iteration", 0.92, "automatic", now, 1);
